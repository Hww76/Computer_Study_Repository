...
fn main() {
    ...
    // 打印出来默认都是十进制，如果希望有不同的打印方法，则可以查看否则类型不同的数据无法进行运算文档![formatting](https://doc.rust-lang.org/rust-by-example/hello/print/fmt.html)
    ...
    // 这不是一个错误，而是一个运行时的判断
    assert_eq!(false, boolean); // 判断是否相同

    /*
        报错记录： assert_eq!(true, boolean == true); // 判断是否相同
        four integers: bin: 1024, oct: 1024, dec: 1024, hex: 1024
        thread 'main' panicked at ex6.rs:29:5:
